...
class SemVer(VersioningSchemeImpl):
    ...
    def cmd_bump_major(self, context: Context):
        return create_version(context, scheme_procedures.version_bump_major)

    def cmd_bump_minor(self, context: Context):
        return create_version(context, scheme_procedures.version_bump_minor)

    def cmd_bump_patch(self, context: Context):
        return create_version(context, scheme_procedures.version_bump_patch)

    def cmd_bump_prerelease_type(self, context: Context):
        return create_version(context, scheme_procedures.version_bump_qualifier)

    def cmd_bump_prerelease(self, context: Context):
        return create_version(context, scheme_procedures.version_bump_prerelease)

    def cmd_bump_to_release(self, context: Context):
        return create_version(context, scheme_procedures.version_bump_to_release)

    def cmd_bump_to(self, context: Context):
        return create_version(context, scheme_procedures.VersionSet(context.args['<version>']))
...
def create_version_tag(command_context: CommandContext,
    ...
    if latest_branch_version is not None:
        ...
        template_version_info = context.versioning_scheme.parse_version_info(
        ...
        new_version = semver.format_version(
            ...
            prerelease=str(
                global_sequence_number + 1) if context.config.tie_sequential_version_to_semantic_version and global_sequence_number is not None else template_version_info.prerelease,
    ...
    if new_version_info.major != selected_branch_base_version_info.major or new_version_info.minor != selected_branch_base_version_info.minor:
        result.fail(os.EX_USAGE,
                    ...
                    .format(new_version=repr(new_version),
                            branch_version=repr(
                                "%d.%d" % (
                                selected_branch_base_version_info.major, selected_branch_base_version_info.minor)))
                    )
...
def create_version(context: Context,
                   operation: Callable[[VersionConfig, Optional[str], Optional[int]], Result]) -> Result:
